...
export default function PackageManager() {
  ...
  const loadData = async () => {
    try {
      const [pkgs, cats, carCats] = await Promise.all([
        api("/packages"),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        carPrices: Object.fromEntries(
          Object.entries(form.carPrices).map(([k, v]) => [k, Number(v)])
        ),
          ...
          <div className="flex-1">
            ...
            <div className="mt-4 relative max-w-md">
              <input 
                ...
                className="w-full pl-4 pr-10 py-2.5 bg-white border border-slate-200 rounded-xl outline-none focus:border-blue-500 font-medium text-sm transition-all shadow-sm"
              />
              ...
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  {carCategories.map(cat => (
                    <th key={cat.id} className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">{cat.name}</th>
                  ))}
                  <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">Base</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredPackages.map(pkg => (
                  <tr key={pkg.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    {/* Category Prices */}
                    {carCategories.map(cat => {
                      const price = pkg.carPrices?.[cat.id]
                      return (
                    ...
                    <td className="px-6 py-4">
                      <div className="font-black text-slate-400 text-xs flex items-center">
                        ₹{pkg.price}
                      </div>
                    </td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-4 border-t border-slate-100 mt-4">
                      <label className="block text-[10px] font-black text-slate-900 uppercase tracking-widest mb-3 ml-1">Vehicle Category Pricing</label>
                      <div className="space-y-3">
                        {carCategories.map(cat => (
                          <div key={cat.id} className="flex items-center justify-between gap-4 bg-slate-50 p-3 rounded-xl border border-slate-200">
                            <div className="flex flex-col">
                              <span className="text-xs font-black text-slate-700">{cat.name}</span>
                              <span className="text-[9px] font-bold text-slate-400">{cat.capacity} Seater</span>
                            </div>
                            <div className="relative w-32">
                              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">₹</span>
                              <input
                                type="number"
                                value={form.carPrices[cat.id] || ""}
                                onChange={(e) => setForm({
                                  ...form,
                                  carPrices: { ...form.carPrices, [cat.id]: e.target.value }
                                })}
                                className="w-full pl-6 pr-3 py-2 bg-white border border-slate-200 rounded-lg outline-none focus:border-emerald-500 font-black text-sm"
                                placeholder="Price"
                              />
                            </div>
                          </div>
